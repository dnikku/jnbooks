...
def PI(samples, prob_percent):
    lo, hi = jnp.percentile(samples,
                            q=jnp.array([(100. - prob_percent) / 2, (100. + prob_percent) / 2]))
...
def samples_grid(p_grid, posterior, num, **kwargs):
    sample = r_sample(dist.Categorical(posterior), num, **kwargs)
    return p_grid[sample]
